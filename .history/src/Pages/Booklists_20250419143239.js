...
const Booklists = () => {
  const { toggleWishlist, wishlist, loading: wishlistLoading, error: wishlistError } = useContext(WishlistContext);
  ...
  const handleToggleWishlist = async (bookId) => {
    ...
    try {
      ...
      setSuccess(isAdded ? 'Book added to wishlist!' : 'Book removed from wishlist!');
        ...
        {books.map((book) => (
          <div key={book._id} className="book-card">
            <div className="heart-container">
              <FaHeart
                className={`heart-icon ${wishlist.includes(book._id) ? 'wished' : 'not-wished'}`}
