...
export const WishlistProvider = ({ children }) => {
  ...
  const fetchWishlist = useCallback(async () => {
    ...
    try {
      ...
      const API_URL = `${process.env.REACT_APP_API_URL || 'https://bookverse-app-project-bymmakok-1.onrender.com'}/api/users/${user.userId}`;
  ...
  const toggleWishlist = async (bookId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      if (!token) throw new Error('No authentication token found. Please log in again.');
      const API_URL = `${process.env.REACT_APP_API_URL || 'https://bookverse-app-project-bymmakok-1.onrender.com'}/api/users/${user.userId}/wishlist`;
